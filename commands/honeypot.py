...
class HoneypotView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message(
                "Only administrators can use this panel.", ephemeral=True
            )
            return False
        return True
    ...
    async def toggle_honeypot(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if enabled:
            ...
            await interaction.response.send_message(
                "No honeypot channel configured. Use Configure Channel first.",
                ephemeral=True
            )
    ...
    async def view_channels(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if honeypot_data and honeypot_data.get("enabled"):
            ...
            await interaction.response.send_message(
                "No honeypot channel is currently enabled.", ephemeral=True
            )
...
class HoneypotCog(commands.Cog):
    ...
    @app_commands.command(
        name="honeypot",
        description="Open the honeypot configuration panel"
    )
    ...
    @app_commands.command(
        name="honeypot_configure",
        description="Configure honeypot channel"
    )
    @app_commands.checks.has_permissions(administrator=True)
    ...
    @app_commands.command(
        name="honeypot_exempt",
        description="Add exemption for user or role"
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def honeypot_exempt(
        ...
        if not user and not role:
            await interaction.response.send_message(
                "Please provide either a user or a role to exempt.",
                ephemeral=True
            )
            return

        if user:
            self.database.add_exemption(
                interaction.guild_id,
                user_id=user.id,
                exempt_bots=exempt_bots
            )
            await interaction.response.send_message(
    ...
    async def honeypot_exemptions(self, interaction: discord.Interaction):
        ...
        if not exemptions:
            await interaction.response.send_message(
                "No exemptions configured.", ephemeral=True
            )
            return
    ...
    async def honeypot_triggers(self, interaction: discord.Interaction, limit: int = 10):
        ...
        triggers = self.database.get_recent_triggers(
            interaction.guild_id,
            limit=limit
        )
        if not triggers:
